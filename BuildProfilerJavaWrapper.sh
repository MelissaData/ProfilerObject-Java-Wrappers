...
Config_FileName1="libmdProfiler.so"
...
Config_FileName2="mdProfilerEnums.h"
...
########################## Main ############################
printf "\n====================== WELCOME TO MELISSA PROFILER OBJECT JAVA WRAPPER =====================\n"
...
# Compile SO
gcc -fpic -c -I$ProjectPath -I$JavaIncludeDirectory1 -I$JavaIncludeDirectory2 mdProfilerJavaWrapper.cpp
gcc -g -shared mdProfilerJavaWrapper.o -o libmdProfilerJavaWrapper.so -L$ProjectPath -lmdProfiler -pthread 

rm mdProfilerJavaWrapper.o

# Compile Jar
javac com/melissadata/*.java &> /dev/null

jar cvf mdProfiler.jar com/melissadata/*.class &> /dev/null

if [ -f "libmdProfilerJavaWrapper.so" ];
then
  script_dir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
  file_path="$script_dir/libmdProfilerJavaWrapper.so"

  printf "\nlibmdProfilerJavaWrapper.so has been successfully generated at:\n$file_path\n"
else
  printf "\nError: was not able to create libmdProfilerJavaWrapper.so\n"
fi

if [ -f "mdProfiler.jar" ];
then
  script_dir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
  file_path="$script_dir/mdProfiler.jar"

  printf "\nmdProfiler.jar has been successfully generated at:\n$file_path\n"
else
  printf "\nError: was not able to create mdProfiler.jar\n"
fi
